...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	...
	# Reject drops outside the playable floor polygon. For "floor" items the
	# anchor point we evaluate is the texture's bottom-center (its visual
	# footprint), not the top-left of the sprite — that matches what the player
	# perceives as "on the floor".
	if data.get("placement_type", "any") != "wall":
		var anchor := _floor_anchor_for(at_position, data)
		if not Helpers.is_inside_floor(_to_world(anchor)):
			return false
...
func _drop_data(at_position: Vector2, data: Variant) -> void:
	...
	# Convert to world coords, clamp the floor anchor (bottom-center of the
	# sprite) inside the room polygon, then convert the corrected sprite-origin
	# back to whatever space the rest of the placement pipeline expects.
	var tex := _get_texture_for_item(item_id)
	if tex and data.get("placement_type", "any") != "wall":
		var tex_size := tex.get_size() * item_scale
		var anchor_offset := Vector2(tex_size.x * 0.5, tex_size.y)
		var world_anchor := _to_world(at_position + anchor_offset)
		var clamped_world: Vector2 = Helpers.clamp_inside_floor(world_anchor)
		var clamped_local := _from_world(clamped_world) - anchor_offset
		at_position = clamped_local
